...
axios.defaults.baseURL = NetConfig.Host;
...
const initNetworkState = () => {
  NetInfo.isConnected.addEventListener("connectionChange", _handleConnectivityChange);
...
const removeNetworkStateListener = () => {
  NetInfo.isConnected.removeEventListener("connectionChange", _handleConnectivityChange);
};
...
const rawFetch = (url, param) => {
  ...
  const { create } = axios;
  const axiosFetcher = create(param);
    ...
    urlTemp,
    {
      ...
      transformRequest: [
        data => {
          ...
          formData.append("file", { uri: data.uri, type: "application/octet-stream", name: data.name });
